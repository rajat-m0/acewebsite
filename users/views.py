        ...
        if request.method == "POST":
            ...
            if enrollment_number is not None and len(str(enrollment_number)) > 0:
                if len(str(enrollment_number)) != 11:
                    errors[
                        "enrollment_number"
                    ] = "Enrollment number must be exactly 11 digits long"
                elif not str(enrollment_number).isdigit():
                    errors[
                        "enrollment_number"
                    ] = "Enrollment number can only contain digits"

            if bool(errors):
                return render(request, "users/signup.html", context={"errors": errors})
            ...
            try:
                ace_user = Profile.objects.create(
                    user=user,
