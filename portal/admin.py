...
from .models import Category, Submission, Task
...
class SubmissionAdmin(admin.ModelAdmin):
    ...
    def review_link(self, instance):
        # print(self.review)
        url = reverse("admin:review_review_add") + "?submission_id={}".format(
            instance.id
        )
        try:
            if self._user:
                ...
                try:
                    self.review = Review.objects.get(
                        user__user=self._user, submission=instance
                    )
                except ObjectDoesNotExist:
                    if Review.objects.filter(submission=instance).exists():
                        return format_html(
                            "<a href='{}' >Already Reviewed</a>".format(url)
                        )
            ...
            if self.review:
                ...
                url = reverse(
                    f"admin:{app_label}_{model_name}_change", args=[self.review.id]
                )
                return format_html(
                    "<a href='{}' >Already Reviewed by you</a>".format(url)
                )
    ...
    def change_view(self, request, object_id, form_url="", extra_context=None):
        ...
        try:
            review = Review.objects.get(
                submission_id=object_id, user__user=request.user
            )
            self.review = review
        except ObjectDoesNotExist:
            self.review = None
    ...
    def submission_link(self, obj):
        ...
        return format_html(
            '<a href="{}">{}</a>', obj.submission_url, obj.submission_url
        )
