...
from ace.settings import (MEDIA_ROOT, SELECTION_END_DATE,
                          SELECTION_RESULT_DATE, SELECTION_START_DATE)
from portal.models import Category, Submission, Task
from review.models import SelectionResult
from users.models import Profile

from .drive import create_folder, refresh_token, upload_drive_task
...
def serve_tasks(request, id):
    ...
    today = localtime(now())
    if (
        (today < SELECTION_START_DATE) or (today > SELECTION_END_DATE)
    ) and not request.user.is_superuser:
        return redirect("portal_home")

    tasks = Task.objects.filter(category=category).order_by("difficulty_value")
    submissions = Submission.objects.filter(
        user__user=request.user, task_submitted=True
    ).values_list("task_id", flat=True)
    # return render(request, 'portal/Tasks_2/index.html', {'category': category, 'tasks': tasks})
    return render(
        request,
        "portal/categories/task.html",
        {"tasks": tasks, "category": category, "submissions": submissions},
    )
...
def home(request):
    if Profile.objects.filter(user=request.user).exists():
        ...
        if (
            ...
            return render(
                request,
                template_name="portal/{}countdown.html".format(name),
                context=context,
            )
        else:
            ...
            return render(
                request, "portal/categories/index.html", {"categories": categories}
            )
            ...
            return render(
                ...
                "portal/main.html",
                {
                    ...
                    "submission_deadline": SELECTION_END_DATE,
                },
            )
...
def submit_task(request):
    ...
    if not request.POST.get("task_id", None) or not request.FILES.get(
        "task_file", None
    ):
    ...
    submission_obj, created = Submission.objects.get_or_create(
        user=ace_profile_obj, task=task_obj
    )
    ...
    try:
        if not ace_profile_obj.submission_folder:
            ace_profile_obj.submission_folder = create_folder(
                "{}#{}#{}".format(ace_profile_obj.id, user.first_name, user.email),
                description="Submission folder of {}".format(user.email),
            )
    ...
    t = threading.Thread(
        target=upload_drive_task,
        args=(
            filename,
            tmp_storage_file,
            submission_obj,
            ace_profile_obj.submission_folder,
        ),
    )
    t.start()
...
def selection_results(request):
    ...
    return render(request, "portal/results.html", context={"profiles": profiles})
