...
def get_profiles():
    return Profile.objects.filter(Q(profilestatus__isnull=True) | Q(profilestatus__contact_added=False)).filter(id__gte=50).order_by('id')
...
def mark_added(request):
    ...
    for profile in profiles:
        profilestatuses.append(ProfileStatus(
            profile=profile,
            contact_added=True
        ))
