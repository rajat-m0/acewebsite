...
class ReviewAdmin(admin.ModelAdmin):
    ...
    list_display = ('id', linkify('user'), linkify('submission'), 'feedback', 'rating', 'is_selected')
    search_fields = ['submission__user__user__first_name', 'submission__user__user__last_name', 'submission__user__email_id', 'submission__task__task_name', 'feedback', ]
...
django.jQuery(function () {
    ...
    django.jQuery("#id_submission").select2();
            ...
            """,
            'fields' : (
                'user', 'submission', 'feedback', 'rating', 'is_selected'
            )
        }),
    )
    ...
    def has_change_permission(self, request, obj=None):
        if obj is not None:
            # print(Profile, obj)
            return request.user.username == 'chirgjin' or Profile.objects.filter(user=request.user, pk=obj.user.id).exists()
